...
> PostgreSQL 业务数据 + DashVector 向量检索 + Redis 三层缓存
...
backend/
├── config/
│   └── __init__.py              # 全局配置 (PG/Redis/DashVector 连接参数)
├── db/
...
**Collection**: `influencer_vectors`（可通过 `DASHVECTOR_COLLECTION` 覆盖）

| 字段 | 类型 | 维度 | 说明 |
|------|------|------|------|
| `id` | String | — | 映射 PG `internal_id` |
| `vector` | Float Vector | `EMBEDDING_DIM` | 文本/视觉融合后的统一 embedding |
| `followers` | Integer | — | 标量过滤: 粉丝数 |
| `region` | String | — | 标量过滤: 地区 |
| `gender` | String | — | 标量过滤: 性别 |
...
### DashVector 混合检索 (`milvus.MilvusManager` 兼容层)
...
results = milvus_mgr.hybrid_search(
    ...
    query_vector=[...],  # EMBEDDING_DIM 维
    scalar_filters={"region": ["上海"], "gender": "女"},
    top_k=100,
)

# 兼容旧接口：v_overall_style / v_scene / v_face 会映射到同一 DashVector embedding 字段
```
...
| 服务 | 端口 | 用途 |
|------|------|------|
| PostgreSQL | 5432 | 关系型数据库；生产推荐阿里云 RDS 内网地址 |
| Redis | 6379 | 缓存 + 消息代理；生产推荐阿里云 Redis/Tair 内网地址 |
| DashVector | HTTPS endpoint | 向量数据库；使用 `DASHVECTOR_ENDPOINT` 和 `DASHVECTOR_API_KEY` |
...
- 时序融合风格向量计算
- DAG 编排引擎将处理结果写入 DashVector Collection
